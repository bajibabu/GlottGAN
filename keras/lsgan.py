...
def generator_containing_discriminator(generator, discriminator):
    ...
    CGAN_model = Model(input=[gen_input, aux_input], output=[x], name="CGAN")
    return CGAN_model
...
def train(BATCH_SIZE, X_train, Y_train):
    ...
    discriminator_on_generator.compile(loss='mse', optimizer=g_optim)

    discriminator.trainable = True
    discriminator.compile(loss='mse', optimizer=d_optim)
...
if __name__ == "__main__":
    ...
    if args.mode == "train":
        data, ac_data = load_data(args.data_dir, num_files=178)
